...
// Observation status is deterministic (`server/dashboardAnalysis.js`), so the tint is a lookup,
// never a judgement made at render time.
const STATUS_STYLES = {
...
/**
 * Presentational only, exactly as `FinanceInsightsPanel` is: every piece of state lives in
 * `Dashboard.jsx` so a tab change cannot strand a half-sent question. The record it renders is
 * written entirely by `server/dashboardInsightGeneration.js` — titles, evidence, ordering and every
 * number are deterministic, and only `headline` and each `body` are model wording.
 */
export default function DashboardInsightsPanel({
  hasAiKey, record, chatMessages = [],
  staleReason = null, scopeLabel,
  ...
  const hasResult = !!record?.headline
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <h2 className="text-[15px] font-semibold text-gray-900">Dashboard Insights</h2>
      ...
      {/* Two dates, not one. Balances are current as of today; only the change decomposition is
          scoped to the period chip — the same split the cards above make. */}
      <p className="mb-3 text-xs leading-relaxed text-gray-400">
        Balances as of {dayjs(record?.asOf ?? undefined).format('MMM D, YYYY')} · change over{' '}
        {record?.periodLabel ?? scopeLabel}.
      ...
      {hasAiKey && generating && !hasResult && (
        <div className="flex flex-col gap-2.5" aria-hidden>
          <div className="animate-pulse rounded-xl border border-gray-100 bg-gray-50/60 p-4">
            <div className="mb-2.5 h-2.5 w-4/5 rounded bg-gray-200" />
      ...
      {hasAiKey && !hasResult && !generating && !insightsError && (
        <div className="py-5 text-center">
          <p className="text-sm text-gray-500">Read your liquid net worth back to yourself.</p>
          <p className="mt-1 text-xs leading-relaxed text-gray-300">
            Generate splits the period's change into what you saved and what the markets did, checks
            how long your cash would last, and flags anything the ledger cannot account for.
          </p>
        </div>
      )}
      ...
      {hasResult && (
        <>
          {/* Two ways to go stale, said differently, because the fix a reader needs is different.
              A scope change is something they did on purpose; a data change is one they made
              somewhere else and would not connect to this panel on their own. */}
          {staleReason && (
            <div className="mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs leading-relaxed text-amber-800">
              {staleReason === 'scope' ? (
                <>
                  These cover <strong className="font-semibold">{record?.periodLabel ?? 'a different period'}</strong>,
                  not what you are viewing now. Follow-ups still answer against the older period.
                </>
              ) : (
                <>
                  Your data has changed since these were generated, so the figures below no longer
                  match the cards above. Follow-ups still answer against the older numbers.
                </>
              )}
              <button
                onClick={onGenerate}
              ...
              >
                {staleReason === 'scope' ? `Re-analyze for ${scopeLabel}` : 'Re-analyze with the current numbers'}
              </button>
            </div>
          )}
          ...
          <div className={`flex flex-col gap-3 transition-opacity ${generating ? 'opacity-50' : ''}`}>
            <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-4">
              <p className="text-[13px] leading-relaxed text-gray-700">{record.headline}</p>
            </div>
          ...
          <div className="mt-4 border-t border-gray-100 pt-3.5">
            ...
            <form onSubmit={onSendChat} className="flex gap-2">
              <input
                ...
                maxLength={2000}
                placeholder="Ask about your balances…"
                disabled={!hasAiKey || chatLoading}
                aria-label="Ask a follow-up about your liquid net worth"
